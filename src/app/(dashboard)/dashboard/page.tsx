...
async function StatsSection() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <StatCard title="Pembayaran Tertunda" value={stats.pendingPayments} description="Invoice belum dibayar" icon={<Clock className="h-5 w-5 text-primary" />} />
      <StatCard title="Stok Menipis" value={stats.lowStockProducts} description="Produk perlu restok" icon={<AlertTriangle className="h-5 w-5 text-primary" />} />
    </div>
  );
}
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {pending.unpaidInvoices.map((inv) => (
                <Link key={inv.id} href={`/invoices/${inv.id}`} className="flex items-center justify-between rounded-lg border p-2 hover:bg-muted/50 text-sm">
        ...
        {pending.incompleteVisits.length > 0 && (
          <div>
            <p className="text-sm font-medium mb-2 flex items-center gap-2"><Clock className="h-4 w-4 text-blue-500" />Kunjungan Belum Selesai ({pending.incompleteVisits.length})</p>
            <div className="space-y-2">
              {pending.incompleteVisits.map((v) => (
                <Link key={v.id} href={`/visits/${v.id}`} className="flex items-center justify-between rounded-lg border p-2 hover:bg-muted/50 text-sm">
        ...
        {pending.lowStockProducts.length > 0 && (
          <div>
            <p className="text-sm font-medium mb-2 flex items-center gap-2"><ShoppingCart className="h-4 w-4 text-orange-500" />Stok Menipis ({pending.lowStockProducts.length})</p>
            <div className="space-y-2">
              {pending.lowStockProducts.map((p) => (
                <Link key={p.id} href="/master/stock" className="flex items-center justify-between rounded-lg border p-2 hover:bg-muted/50 text-sm">
                  <span>{p.name}</span>
            ...
            {recent.recentVisits.map((visit: any) => (
              <div key={visit.id} className="flex items-center justify-between rounded-lg border p-3">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">{visit.customer.name} &mdash; {visit.pet.name}</p>
                </div>
